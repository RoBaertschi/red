...
setup :: proc() -> Renderer {
    ...
    r.ibuf = sg.make_buffer({type = .INDEXBUFFER, usage = .DYNAMIC, size = DEFAULT_BUFFER_SIZE * size_of(u16)})
...
update_buffers :: proc(r: ^Renderer) {
    ...
    sg.update_buffer(r.vbuf, {size = len(r.vertices) * size_of(Vertex), ptr = raw_data(r.vertices)})
