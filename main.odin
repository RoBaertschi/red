...
main :: proc() {
	status, desc, code := glfw_init(
		{title = "red", width = 800, height = 600, state = {ctx = context}},
	)
	...
	buffer := sg.make_buffer({data = {ptr = &vertices, size = size_of(vertices)}})
			...
			sg.begin_pass(
				{
					...
					action = {
						colors = {0 = {load_action = .CLEAR, clear_value = {0.2, 0.2, 0.2, 1}}},
					},
				},
			)
			defer sg.end_pass()
